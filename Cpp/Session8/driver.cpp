...
int main()
{
    ...
    Employee e1(id,salary,name);
    Employee e2(e1);

    Manager m1 (id,salary,name);  // statically allocated in stack

    Employee* manager_ptr = new Manager(id,salary,name);
    Employee* manager_ptr2 = new Employee(id2,salary2,name2);  //dynamically allocated in heap
    
    std::cout << "name: " << manager_ptr->getName() << std::endl;

    std::vector<Employee*> employees;
    employees.push_back(manager_ptr);
    employees.push_back(manager_ptr2);
    displaySalaries(employees);
    delete manager_ptr;
    delete manager_ptr2;    // to remove from heap
