...
int main( void )
 {
	...
	if(queue != NULL)
	{
		...
		xTaskCreate(task2,"task2",300,NULL,4,NULL);
	}
...
void task1(void *pvParameter){
	...
	while(1){
		...
		GPIOC->ODR ^= (1<<13);
		xQueueSendToBack(queue,control, 150/portTICK_PERIOD_MS);
	}
...
void task2(void *pvParameter){
	...
	while(1){
		...
		xQueueReceive(queue, &received_control, 150/portTICK_PERIOD_MS);
